...
const Modal: FunctionComponent<Props> = ({ open, onClose, children }) => {
  ...
  return (
    <div>
      ...
      >
        <div
          className="rounded-lg relative p-11 max-h-full overflow-y-auto animate__animated animate__zoomIn animate__faster mt-36 bg-white z-40"
        ...
        >
          <div
            ...
            onClick={onClose}
          >
            <Cross className='absolute top-8 rounded opacity-60 z-40' color={colors.darkgrey} />
          </div>
          <div>{children}</div>
        </div>
      </div>
