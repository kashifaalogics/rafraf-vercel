...
import { A, H1, H2, H3, H4 } from "@components/typography";
import { Button, Container, Image } from "@components/ui";
import { getConfig } from "@framework/api/config";
import { getStaticCategories } from "@framework/categories";
...
export default function AboutUs({
  ...
  return (
    <>
      <Head>
        <link rel="canonical" href={`https://rafraf.com/${rtl? "ar" : "en"}/aboutus`} />
        <title>{t("static/aboutus:title")} | {t("common:rafraf")}</title>
      </Head>
      <div>
        <div className="py-12">
          {/* heading */}
          <Container className="pb-9">
            <H2 className="text-2xl">{t("static/aboutus:title")}</H2>
          </Container>

          {/* top banner */}
          <div className="w-full relative">
            <Image
              src={"/images/aboutus/topbanner.jpg"}
              alt="قطع غيار سيارات"
              layout="fill"
              objectFit="cover"
            />
              ...
              <Container className="py-9">
                <section className="w-full md:w-1/2">
                  <H4 className="text-white mb-3">
                    {t("static/aboutus:topbanner.title")}
                  </H4>
                  <div className="text-white">
                    {t("static/aboutus:topbanner.text")}
                  </div>
                  <div className="flex flex-wrap gap-7 mt-8 ">
                    <A className="w-full md:w-auto" href="https://apps.apple.com/sa/app/rafraf-for-cars-parts/id1504242621#?platform=iphone" target="_blank">
                      <Button className="w-full px-4 py-3">
                        {t("static/aboutus:topbanner.download")}
                      </Button>
                    </A>

                    <A className="w-full md:w-auto" href="https://play.google.com/store/apps/details?id=com.rafraf.store&hl=ar" target="_blank">
                      <Button className="w-full px-4 py-3">
                        {t("static/aboutus:topbanner.browse")}
                      </Button>
                    </A>
                  </div>
                </section>
          ...
          <Container>
            <div className="flex flex-wrap my-16 gap-8">
              ...
              <div className="flex-1">
                <H3 className="pb-3">
                  {t("static/aboutus:about.identity.title")}
                </H3>
                <p className="pb-7">
                  {t("static/aboutus:about.identity.text")}
                </p>
                <H3 className="pb-3">
                  {t("static/aboutus:about.vision.title")}
                </H3>
                <p className="pb-7">{t("static/aboutus:about.vision.text")}</p>
                <H3 className="pb-3">
                  {t("static/aboutus:about.standards.title")}
                </H3>
                <p className="pb-7">
                  {t("static/aboutus:about.standards.text")}
                </p>
              </div>
            </div>
          </Container>
          ...
          <div className="w-full relative">
            <Image
              src={"/images/aboutus/bottombanner.jpg"}
              alt="قطع غيار سيارات"
              layout="fill"
              objectFit="cover"
            />
            ...
            >
              <Container className="py-9">
                <section className="w-full md:w-1/2">
                  <H4 className="text-white mb-3">
                    {t("static/aboutus:values.title")}
                  </H4>
                  <div className="text-white flex flex-col gap-4">
                    <div>
                      <strong>
                        {t("static/aboutus:values.passion.title")}
                      </strong>
                      : {t("static/aboutus:values.passion.text")}
                    </div>
                    <div>
                      <strong>{t("static/aboutus:values.trust.title")}</strong>:{" "}
                      {t("static/aboutus:values.trust.text")}
                    </div>
                    <div>
                      <strong>
                        {t("static/aboutus:values.quality.title")}
                      </strong>
                      : {t("static/aboutus:values.quality.text")}
                    </div>
                    <div>
                      <strong>
                        {t("static/aboutus:values.integrity.title")}
                      </strong>
                      : {t("static/aboutus:values.integrity.text")}
                    </div>
                    <div>
                      <strong>
                        {t("static/aboutus:values.commitment.title")}
                      </strong>
                      : {t("static/aboutus:values.commitment.text")}
                    </div>
                  </div>
                </section>
              </Container>
          ...
          <Container className="py-16">
            <H4 className="text-center">{t("static/aboutus:social.title")}</H4>

            <div className="flex flex-wrap justify-center gap-28 mt-9">
